...
Heatmap   <- matrix(NA, nrow = 8, ncol = 8)
Heatmap_m <- matrix(NA, nrow = 8, ncol = 8)
Heatmap_M <- matrix(NA, nrow = 8, ncol = 8)
...
DL <- log10(6.3*(10^4))            ## Detection limit for low-sensitivity
...
Scenario <- matrix(NA, nrow = iteration, ncol = screening)
...
Heatmap[iii,jjj]   <- mean(Scenario[,screening])
Heatmap_m[iii,jjj] <- quantile(Scenario[,screening],0.025)
Heatmap_M[iii,jjj] <- quantile(Scenario[,screening],0.975)
...
Heatmap_high <- data.frame(R0=rep(seq(1.5,5.0,by=0.5),times=8),Proportion=rep(seq(20,90,by=10),each=8),Value=as.vector(Heatmap),Min=as.vector(Heatmap_m),Max=as.vector(Heatmap_M))

write.xlsx(Heatmap_high,"Heatmap_high.xlsx")
